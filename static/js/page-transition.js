(function(){
  ...
  function handleNavSwap(event){
    ...
    const currentIndex = navigation.navIndex(window.location.pathname);
    const targetIndex = navigation.navIndex(url.pathname);
    ...
    navigation.setNavActiveNow(link);
    navigation.moveNavSliderTo(link, false);
  ...
  if(document.readyState === 'loading'){
    ...
    navigation.bindNavIndicatorHover();
    navigation.updateNavIndicator(false);
    if(shouldBoot) runBootSequence();
    else { show(); window.requestAnimationFrame(function(){ window.setTimeout(hide, 120); }); }
  }

  window.addEventListener('resize', function(){
    window.requestAnimationFrame(function(){ navigation.updateNavIndicator(true); });
  });

  window.addEventListener('orientationchange', function(){
    window.setTimeout(function(){ navigation.updateNavIndicator(true); }, 260);
  });
